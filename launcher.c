...
int main(int argc, char *argv[])
{
    ...
    /* Pre-compute bind-mount destination paths. */
    snprintf(dev_dst,  sizeof(dev_dst),  "%s/dev",  mountpoint);
    snprintf(proc_dst, sizeof(proc_dst), "%s/proc", mountpoint);
    ...
    if (child == 0) {
         ...
         */
        if (bind_mount("/dev",  dev_dst) < 0)
            fprintf(stderr, "launcher: warning: /dev bind mount failed\n");
        if (bind_mount("/proc", proc_dst) < 0)
            fprintf(stderr, "launcher: warning: /proc bind mount failed\n");
